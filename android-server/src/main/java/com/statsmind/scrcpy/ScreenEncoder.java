...
import com.statsmind.scrcpy.wrappers.SurfaceControl;
...
public class ScreenEncoder implements Connection.StreamInvalidateListener, Runnable {
    ...
    private final ByteBuffer headerBuffer = ByteBuffer.allocate(13);
        ...
        while (!consumeStreamInvalidation() && !eof && connection.hasConnections()) {
            ...
            try {
                ...
                if (outputBufferId >= 0) {
                    ...
                    if (videoSettings.isSendFrameMeta()) {
                        writeFrameMeta(bufferInfo, codecBuffer.remaining());
                    }

                    ByteBuffer msg = ByteBuffer.allocate(1 + codecBuffer.remaining());
                    msg.put((byte) ControlMessage.TYPE_VIDEO_STREAM);
                    msg.put(codecBuffer);
                    msg.rewind();
                    connection.send(msg);
                }
    ...
    private void writeFrameMeta(MediaCodec.BufferInfo bufferInfo, int packetSize) throws IOException {
        ...
        Ln.d("ScreenEncoder::writeFrameMeta");
        /*
         * 复用 headerBuffer 无疑是效率最高的，这块就不需要先转化为 FrameMetaMessage 了
         */
        headerBuffer.put((byte) ControlMessage.TYPE_FRAME_META);
