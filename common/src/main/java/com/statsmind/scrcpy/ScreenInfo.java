...
public final class ScreenInfo {
     ...
     */
    private Rect contentRect; // device size, possibly cropped
     ...
     */
    private Size unlockedVideoSize;
    ...
    public static ScreenInfo computeScreenInfo(DisplayInfo displayInfo, VideoSettings videoSettings) {
        ...
        if (crop != null) {
            ...
            if (!contentRect.intersect(crop)) {
                ...
                Log.w(TAG, "Crop rectangle (" + formatCrop(crop) + ") does not intersect device screen (" + formatCrop(deviceSize.toRect()) + ")");
    ...
    public ByteBuffer toBuffer() {
        try (ByteBufferWriter writer = new ByteBufferWriter()) {
            writer.putRect(contentRect);
            writer.putSize(unlockedVideoSize);
            writer.putByte((byte) getVideoRotation());

            return writer.toBuffer();
        }
    }

    public static ScreenInfo fromBuffer(ByteBuffer buffer) {
        ScreenInfo msg = new ScreenInfo();

        try (ByteBufferReader reader = new ByteBufferReader(buffer)) {
            msg.contentRect = reader.getRect();
            msg.unlockedVideoSize = reader.getSize();
            msg.deviceRotation = reader.getByte();
        }
